...
Cpp-Mental-Models 是一个模块化的 C/C++ 学习仓库，旨在通过可运行示例、物理类比和全局 CMake 构建系统，讲清核心语言机制与系统级机制。

它覆盖内存布局、移动语义、智能指针、静态与动态链接、面向对象编程、可调用对象、命名空间、构建类型，以及裸机交叉编译。这个项目展示了我组织技术知识、编写可复现 C++ 示例，以及清晰传达底层概念的能力。
